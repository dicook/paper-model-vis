...
ggplot(data=subset(best$output,  node == 1), aes(x, y)) +
  geom_point(aes(shape=class, colour=class), data=w) + sqr + col_d + 
  geom_contour(aes(z=pred), breaks = c(0.1, 0.25, 0.5, 0.75, 0.9)) + nolab
...
ggplot(data=best$hidden, aes(x, y)) + 
  geom_contour(aes(z=pred, group=node), colour="grey50", size=2, breaks = 0.5) +
