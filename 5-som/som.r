...
init <- cbind(
  expand.grid(
    seq(min(w5[ ,1]), max(w5[ ,1]), length.out = 10),
    seq(min(w5[ ,2]), max(w5[ ,2]), length.out = 3)
  ),
  matrix(rep(0,30 * 3), ncol = 3)
)
init.grid <- matrix(ncol = 2,
  c(1:9, 11:19, 21:29, 1:10, 11:20, 2:10, 12:20, 22:30, 11:20, 21:30))
...
right <- som_iterate(w5, grid = somgrid(10, 3), radius = c(3, 0),
  stepsize = 1, nsteps = 100, init = init)
#right <- som_iterate(w5, grid = somgrid(3, 3), radius=c(3, 0), stepsize=1, nsteps=100, init=init)
ggobi(right, cls = wine$type, map = init.grid)
...
ggplot(right_sum, aes(step, dist_mean)) +
  geom_line(aes(y = value, group = oid), colour = scales::alpha("grey10", 0.2),
    data = distances) +
