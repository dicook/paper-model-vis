...
ggplot(grid, aes(color, phenols, colour=type)) + geom_point(data=winer) + geom_contour(data = grid, aes(x=color, y=phenols, z=.ADVANTAGE, colour=type), size = 0.5, breaks=0.05) + col + coord_equal() #+ scale_z_continuous(breaks=0.05) 
ggsave(file = "class-2d-boundary.pdf", width=5, height=4)


w <- rescaler(read.csv("../6-nnet/wiggly.csv")[, -1], "range")
